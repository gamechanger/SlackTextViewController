...
@property (nonatomic, strong) NSLayoutConstraint *leadingLeftButtonWC;
@property (nonatomic, strong) NSLayoutConstraint *leadingLeftButtonHC;
@property (nonatomic, strong) NSLayoutConstraint *leftMarginWC;
@property (nonatomic, strong) NSLayoutConstraint *leadingLeftButtonBottomMarginC;
@property (nonatomic, strong) NSLayoutConstraint *trailingLeftButtonWC;
@property (nonatomic, strong) NSLayoutConstraint *trailingLeftButtonHC;
@property (nonatomic, strong) NSLayoutConstraint *leftMargin2WC;
@property (nonatomic, strong) NSLayoutConstraint *trailingLeftButtonBottomMarginC;
...
- (void)slk_commonInit
{
    ...
    [self addSubview:self.leadingLeftButton];
    [self addSubview:self.trailingLeftButton];
    ...
    [self slk_registerTo:self.leadingLeftButton.imageView forSelector:@selector(image)];
    [self slk_registerTo:self.trailingLeftButton.imageView forSelector:@selector(image)];
...
{
    if (!_leadingLeftButton) {
        _leadingLeftButton = [UIButton buttonWithType:UIButtonTypeSystem];
        _leadingLeftButton.translatesAutoresizingMaskIntoConstraints = NO;
        _leadingLeftButton.titleLabel.font = [UIFont systemFontOfSize:15.0];
    }
    return _leadingLeftButton;
}

- (UIButton *)trailingLeftButton
{
    if (!_trailingLeftButton) {
        _trailingLeftButton = [UIButton buttonWithType:UIButtonTypeSystem];
        _trailingLeftButton.translatesAutoresizingMaskIntoConstraints = NO;
        _trailingLeftButton.titleLabel.font = [UIFont systemFontOfSize:15.0];
    }
    return _trailingLeftButton;
}
...
- (void)slk_setupViewConstraints
{
    NSDictionary *views = @{@"textView": self.textView,
                            @"leadingLeftButton": self.leadingLeftButton,
                            @"trailingLeftButton": self.trailingLeftButton,
    ...
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"H:|-(left)-[leadingLeftButton(0)]-(left)-[trailingLeftButton(0)]-(<=left)-[textView]-(right)-[rightButton(0)]-(right)-|" options:0 metrics:metrics views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=0)-[leadingLeftButton(0)]-(0@750)-|" options:0 metrics:metrics views:views]];
    [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=0)-[trailingLeftButton(0)]-(0@750)-|" options:0 metrics:metrics views:views]];
    ...
    self.leadingLeftButtonWC = [self slk_constraintForAttribute:NSLayoutAttributeWidth firstItem:self.leadingLeftButton secondItem:nil];
    self.leadingLeftButtonHC = [self slk_constraintForAttribute:NSLayoutAttributeHeight firstItem:self.leadingLeftButton secondItem:nil];
    self.leadingLeftButtonBottomMarginC = [self slk_constraintForAttribute:NSLayoutAttributeBottom firstItem:self secondItem:self.leadingLeftButton];

    self.trailingLeftButtonWC = [self slk_constraintForAttribute:NSLayoutAttributeWidth firstItem:self.trailingLeftButton secondItem:nil];
    self.trailingLeftButtonHC = [self slk_constraintForAttribute:NSLayoutAttributeHeight firstItem:self.trailingLeftButton secondItem:nil];
    self.trailingLeftButtonBottomMarginC = [self slk_constraintForAttribute:NSLayoutAttributeBottom firstItem:self secondItem:self.trailingLeftButton];
...
- (void)slk_updateConstraintConstants
{
    ...
    if (self.isEditing)
    {
        ...
        self.leadingLeftButtonWC.constant = zero;
        self.leadingLeftButtonHC.constant = zero;
        self.leftMarginWC.constant = zero;
        self.leadingLeftButtonBottomMarginC.constant = zero;
        self.trailingLeftButtonWC.constant = zero;
        self.trailingLeftButtonHC.constant = zero;
        self.trailingLeftButtonBottomMarginC.constant = zero;
        ...
        CGSize leadingLeftButtonSize = [self.leadingLeftButton imageForState:self.leadingLeftButton.state].size;
        CGSize trailingLeftButtonSize = [self.trailingLeftButton imageForState:self.trailingLeftButton.state].size;
        
        if (leadingLeftButtonSize.width > 0) {
            self.leadingLeftButtonHC.constant = roundf(leadingLeftButtonSize.height);
            self.leadingLeftButtonBottomMarginC.constant = roundf((self.intrinsicContentSize.height - leadingLeftButtonSize.height) / 2.0) + self.slk_contentViewHeight / 2.0;
        }

        if (trailingLeftButtonSize.width > 0) {
            self.trailingLeftButtonHC.constant = roundf(trailingLeftButtonSize.height);
            self.trailingLeftButtonBottomMarginC.constant = roundf((self.intrinsicContentSize.height - trailingLeftButtonSize.height) / 2.0) + self.slk_contentViewHeight / 2.0;
        }
        
        self.leadingLeftButtonWC.constant = roundf(leadingLeftButtonSize.width);
        self.leftMarginWC.constant = (leadingLeftButtonSize.width > 0) ? self.contentInset.left : zero;

        self.trailingLeftButtonWC.constant = roundf(trailingLeftButtonSize.width);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    else if ([object isEqual:self.leadingLeftButton.imageView] && [keyPath isEqualToString:NSStringFromSelector(@selector(image))]) {
    ...
    else if ([object isEqual:self.trailingLeftButton.imageView] && [keyPath isEqualToString:NSStringFromSelector(@selector(image))]) {
...
- (void)dealloc
{
    ...
    [self slk_unregisterFrom:self.leadingLeftButton.imageView forSelector:@selector(image)];
    [self slk_unregisterFrom:self.trailingLeftButton.imageView forSelector:@selector(image)];
